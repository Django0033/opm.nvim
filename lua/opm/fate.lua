...
local ODDS_ERROR = "Invalid odds. Use: impossible, nearly_impossible, very_unlikely, unlikely, fifty_fifty, likely, very_likely, nearly_certain, certain"

local function interpret_roll(roll, chart)
...
function M.roll_fate(question, odds_key)
	...
	if not chart then
		return nil, ODDS_ERROR
	end

	local roll = dice.roll_d100()
	local result = interpret_roll(roll, chart)
